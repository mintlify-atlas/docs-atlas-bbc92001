...
`Opscale\NotificationCenter\Services\Actions\EnrichGeoData` enriches an existing `Event` record's payload with geolocation data from [ipgeolocation.io](https://ipgeolocation.io). It requires an API key set via:
